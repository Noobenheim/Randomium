...
public class CustomBlockFeature extends Feature<CustomBlockConfig> {
	public static final CustomBlockFeature CUSTOM_ORE = new CustomBlockFeature(CustomBlockConfig::deserialize);
	
	public CustomBlockFeature(Function<Dynamic<?>, ? extends CustomBlockConfig> configFactoryIn) {
		super(configFactoryIn);
	}
	
	@Override
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random random, BlockPos pos, CustomBlockConfig config) {
		...
		for(int l1 = k; l1 <= k + j1; ++l1) {
			for(int i2 = i1; i2 <= i1 + j1; ++i2) {
				if (l <= worldIn.getHeight(Heightmap.Type.OCEAN_FLOOR_WG, l1, i2)) {
					return this.func_207803_a(worldIn, random, config, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
				}
			}
		}
	...
	protected boolean func_207803_a(IWorld worldIn, Random random, CustomBlockConfig config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
		int i = 0;
								...
								for(int j2 = l; j2 <= k1; ++j2) {
									...
									if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
										...
										if (!bitset.get(k2)) {
											...
											if (config.target.getBlockState().test(worldIn.getBlockState(blockpos$mutableblockpos))) {
